"""
plugins.node20 — Node.js 20 source-file plugin for Apollo
==========================================================

Parses Node.js (``.js`` / ``.mjs`` / ``.cjs``) source files into
Apollo's structured result dict so the knowledge graph can carry
``defines`` / ``calls`` / ``inherits`` edges between Node entities.

Node has no Python-callable AST in the standard library, so this is a
*regex-based* parser. Compared to the more general ``javascript1``
plugin, this one tilts toward server-side Node patterns:

* CommonJS ``require('x')`` is a first-class import shape.
* ``module.exports`` and ``exports.foo`` are surfaced as variables so
  the graph can answer "what does this module export?".
* ``.cjs`` (CommonJS) is recognised in addition to ``.mjs`` / ``.js``.

Both Node and "browser" JS regularly target ``.js``. For a single
project, prefer enabling exactly one of ``node20`` or ``javascript1``
via each plugin's ``config.json``.

For production use a real JS AST (``acorn`` / ``esprima`` /
...
_MODULE_EXPORT_PROP_RE = re.compile(
    r"^\s*(?:module\.)?exports\.(?P<name>[A-Za-z_$][\w$]*)\s*=", re.MULTILINE
)
_NAMED_EXPORT_RE = re.compile(
    r"^\s*export\s+(?:const|let|var|function|class|default)\s+"
    r"(?P<name>[A-Za-z_$][\w$]*)",
    re.MULTILINE,
)
...
class Node20Parser(BaseParser):
    """Regex-based parser for Node.js 20 source files."""
    ...
    def __init__(self, config: Optional[dict] = None) -> None:
        ...
        self._extensions = frozenset(
            ext.lower()
            for ext in (self.config.get("extensions") or [".js", ".mjs", ".cjs"])
        )
    ...
    def _extract_variables(
        ...
        for m in _MODULE_EXPORT_PROP_RE.finditer(source):
            name = m.group("name")
            if name in seen:
                continue
            seen.add(name)
        ...
        # ``export const foo`` / ``export function foo`` / ``export default foo``
        for m in _NAMED_EXPORT_RE.finditer(source):
            name = m.group("name")
            if name in seen:
                continue
            seen.add(name)
