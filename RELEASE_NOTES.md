...
- **Parser Pool + Streaming Index Build (Memory Reduction)** — Major overhaul of the indexing pipeline to drastically reduce peak memory on large repos. [`graph/builder.py`](graph/builder.py) (+664 lines) and [`graph/incremental.py`](graph/incremental.py) (+232 lines) rework parsing into a bounded parser pool with streaming graph emission; [`storage/json_store.py`](storage/json_store.py) (+228 lines) gains streaming save / sweep-in-place support; [`embeddings/embed_queue.py`](embeddings/embed_queue.py) (+213 lines, new) is a back-pressured embedding queue; [`embeddings/embedder.py`](embeddings/embedder.py) refactored for parallel ML passes. New file-text sidecar layout decouples raw text from the graph snapshot. Covered by 10 new phase-tagged test files: [`test_parser_pool_phase2.py`](tests/test_parser_pool_phase2.py), [`test_embedding_storage_phase3.py`](tests/test_embedding_storage_phase3.py), [`test_file_text_sidecar_phase4.py`](tests/test_file_text_sidecar_phase4.py), [`test_streaming_save_phase5.py`](tests/test_streaming_save_phase5.py), [`test_embed_queue_phase6.py`](tests/test_embed_queue_phase6.py), [`test_parallel_ml_phase7.py`](tests/test_parallel_ml_phase7.py), [`test_sweep_in_place_phase8.py`](tests/test_sweep_in_place_phase8.py), [`test_parallel_rehash_phase9.py`](tests/test_parallel_rehash_phase9.py), [`test_index_invariants_phase10.py`](tests/test_index_invariants_phase10.py), and [`test_builder_streaming.py`](tests/test_builder_streaming.py). New benchmarking helpers in [`scripts/bench_index.py`](scripts/bench_index.py) and [`scripts/bench_check.py`](scripts/bench_check.py). See [`docs/work/PLAN_INDEX_MEMORY_AND_CONCURRENCY.md`](docs/work/PLAN_INDEX_MEMORY_AND_CONCURRENCY.md) (1464 lines) for the design and [`docs/INCREMENTAL_REINDEX_GUIDE.md`](docs/INCREMENTAL_REINDEX_GUIDE.md) for the operator-facing notes.
- **UI / Sidebar Refresh (#15)** — Refreshed left navigation and panel layout in [`web/static/app.js`](web/static/app.js) (+185/-106 lines) and [`web/static/index.html`](web/static/index.html) — clearer sidebar hierarchy, version badge moved next to the logo, improved panel toggling.
- **Hybrid Graph + Spatial Design Doc** — New [`docs/HYBRID_GRAPH_SPATIAL.md`](docs/HYBRID_GRAPH_SPATIAL.md) (1196 lines) describing the long-term hybrid graph/spatial-index architecture, plus expanded [`docs/DESIGN.md`](docs/DESIGN.md) (+335 lines across two commits) covering branch-keyed stores and the streaming index pipeline.

### Changes

- **BSL-1.1 Licensing** — Project relicensed under [Business Source License 1.1](licenses/BSL-1.1.txt). New [`LICENSE`](LICENSE) file at root, license header inserted into every first-party `.py` file across `apollo/`, `chat/`, `embeddings/`, `graph/`, `ml/`, `parser/`, `plugins/`, `storage/`, `web/`, `search/`, `scripts/`, `tests/`, and root-level scripts.
- **CI Gates** — New [`.github/workflows/ci.yml`](.github/workflows/ci.yml) (92 lines) running ruff (errors-only) and the new [`scripts/check_license_headers.py`](scripts/check_license_headers.py) license-header checker on every push/PR. [`guides/RELEASE.md`](guides/RELEASE.md) updated to document the local equivalents.
- **Version bump** — All version references updated from v1.3.0 to v1.4.0 (`main.py`, `README.md` badge, `web/static/index.html` sidebar).
